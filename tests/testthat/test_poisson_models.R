...
test_that("Poisson models with increasing complexity do not throw errors", {
    # pe
    expect_equal(r2_mod1_1$R2_pe_ci$R2, 0.0895, tolerance = 0.01)
    # ci
    expect_equal(r2_mod1_2$R2_pe_ci$CI_lower, 0.065, tolerance = 0.01)
    expect_equal(r2_mod1_2$R2_pe_ci$CI_upper, 0.134, tolerance = 0.01)
    # R2 one partvar
    expect_equal(r2_mod1_3$R2_pe_ci$R2, c(0.0895, 0.086), tolerance = 0.01)
    # R2 two partvars
    expect_equal(r2_mod1_4$R2_pe_ci$R2, c( 0.089, 0.086, 0.003, 0.089),
                 tolerance = 0.01)
