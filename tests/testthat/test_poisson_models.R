...
test_that("Poisson models with increasing complexity do not throw errors", {
    ...
    # ci
    expect_equal(r2_mod1_2$R2_pe_ci$CI_lower, 0.072, tolerance = 0.01)
    expect_equal(r2_mod1_2$R2_pe_ci$CI_upper, 0.163, tolerance = 0.01)
    # R2 one partvar
    expect_equal(r2_mod1_3$R2_pe_ci$R2, c(0.103, 0.099), tolerance = 0.01)
    # R2 two partvars
    expect_equal(r2_mod1_4$R2_pe_ci$R2, c(0.103, 0.099, 0.0036, 0.103),
                 tolerance = 0.01)
...
data(BeetlesFemale)
BeetlesFemale$overdisp <- factor(1:nrow(BeetlesFemale))
mod2 <- glmer(Egg ~ Treatment + Habitat + (1|Container) + (1|Population) + (1|overdisp),
              data = BeetlesFemale, family = poisson)
