...
test_that("Gaussian models with increasing complexity give correct R2s", {
    # pe
    expect_equal(r2_mod1_1$R2_pe_ci$R2, 0.62, tolerance = 0.01)
    # ci
    expect_equal(r2_mod1_2$R2_pe_ci$CI_lower, 0.534, tolerance = 0.01)
    expect_equal(r2_mod1_2$R2_pe_ci$CI_upper, 0.689, tolerance = 0.01)
    # R2 one partvar
    expect_equal(r2_mod1_3$R2_pe_ci$R2, c(0.62,  0.177), tolerance = 0.01)
    # R2 two partvars
    expect_equal(r2_mod1_4$R2_pe_ci$R2, c(0.62,  0.177, 0.009, 0.187),
                 tolerance = 0.01)
    # interaction only
    expect_equal(r2_mod1_5$R2_pe_ci$R2, c(0.62, 0.019), tolerance = 0.01)
...
test_that("Predictor manipulation in formula works", {
    expect_equal(r2_mod2_1$R2_pe_ci$R2, 0.595, tolerance = 0.01)
    # all together same as marginal
