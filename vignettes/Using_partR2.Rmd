...
* Inclusive R^2^, which we define as explained variance of a predictor in the fitted response irrespective
of covariances with other predictors. It is calculated as SC^2^ * R^2^, the squared structure coefficient
time the R^2^ of the full model.
...
The workhorse of the package is a single function, `partR2()`. It takes a fitted model from `lme4` (Bates et al. 2015), which can be of either a Gaussian, Poisson or binomial family.
...
There are a few other options on how to deal with interactions, which we detailed in the paper and will
add to this vignette after the review process.
