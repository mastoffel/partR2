...
The goal of `partR2` is to partition the varianced explained in generalised linear mixed models (GLMMs) into variation unique to and shared among predictors (partial R^2^), but it also does a few other things. Here is a quick summary of what you can learn about your mixed model with `partR2`:

* Marginal and conditional R^2^ (based on Nakagawa & Schielzeth, 2013)
* Partial R^2^ unique to each predictor and for each combination of predictors (fixed effects)
...
The output of forestplot is a ggplot object so we can change it using ggplot syntax or assemble multiple plots with packages like `patchwork`. The function also allows to plot partial R2s, slopes or structure coefficients.
...
The first rows show the variances explained by single predictors, while rows 4-8 show variances explained by combinations of predictors. This first row, in this case, is the variance uniquely explained by the interaction `Temperature:Precipitation`. The fourth row shows the variance explained by the `Temperature:Precipitation` interaction together with the main effect of `Temperature`. The seventh row shows the variance explained additive by the two predictors `Temperature` and `Precepitation`.

It is sometimes useful to eyeball the table for jumps in variance explained when predictors are added. However, it is also important to realized the meaning of partial R^2^2 of main effects when interactions are fitted in the model. When a partial main effect is also involved in an interaction, the variance explained by the main effect is usually also explained by the interaction, such that the unique variance explained is often small.
