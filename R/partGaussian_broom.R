...
#'             data = BeetlesBody)
#'
#' R2 <- partGaussian_broom(mod, partvars = c("Sex"),
#'                    R2_type = "marginal", nboot = 20, CI = 0.95)
#' R2
#'
#' @export

partGaussian_broom <- function(mod, partvars = NULL, R2_type = "marginal", nboot = NULL, CI = 0.95){
    ...
    R2_pe <- function(mod) {
        ...
        # old way: VarComps <- lme4::VarCorr(mod)

        tidy_mod <- broom.mixed::tidy(mod)
        VarComps <- tidy_mod[tidy_mod$effect == "ran_pars", ]
        # Residual variance
